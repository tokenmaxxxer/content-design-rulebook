...
PY_SCRIPT="$(mktemp "${TMPDIR:-/tmp}/ab-spec-gate.XXXXXX.py")" || gate_deny "ab-spec-gate" "cannot create temp python file"
INPUT_JSON_FILE="$(mktemp "${TMPDIR:-/tmp}/ab-spec-gate-input.XXXXXX.json")" || gate_deny "ab-spec-gate" "cannot create temp input file"
