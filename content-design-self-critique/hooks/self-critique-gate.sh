...
. "${CLAUDE_PLUGIN_ROOT_CORE:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../../core" && pwd -P)}/hooks/lib/gate-lib.sh" || { echo "self-critique-gate.sh: cannot source gate-lib.sh" >&2; exit 2; }
...
# Payload travels via GATE_INPUT_JSON env var; stdin is claimed by the
# heredoc carrying the python program below.
